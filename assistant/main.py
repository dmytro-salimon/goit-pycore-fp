...
def main():
    book = load_data()
    notes = load_notes()
    print("\n👋 Вітаємо в Personal Assistant!")
    print("💡 Введіть 'help', щоб побачити доступні команди.")
    ...
    while True:
        try:
            ...
            if command in ["close", "exit"]:
                save_data(book)
                save_notes(notes)
                print("👋 До зустрічі! Дані збережено.")
                break
            elif command == "help":
                print(handlers.show_help())
            elif command == "hello":
                ...
                print("❌ Невідома команда. Спробуйте 'help'.")
